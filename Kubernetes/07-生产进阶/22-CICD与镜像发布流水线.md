...
不要一开始就让流水线直接操作生产。按风险分四步建设：

1. **只做验证**：格式检查、单元测试、构建二进制。
2. **产出镜像**：构建、扫描、推送带 commit SHA 的不可变镜像。
3. **自动部署测试环境**：渲染配置、查看 diff、部署、冒烟测试。
4. **受控部署生产**：审批、最小权限身份、兼容迁移、滚动发布、观察和回滚。

每次构建至少保存这些关联：Git commit、镜像仓库和摘要、流水线运行编号、部署环境、操作身份和时间。镜像摘要比可覆盖的标签更能准确标识内容。

流水线失败策略要明确：测试失败不构建；扫描达到团队禁止级别不推送；迁移失败不发布应用；rollout 超时停止并保留现场；冒烟测试失败触发告警和人工判断，不自动回滚数据库。

上线前在测试环境演练三种失败：单元测试失败、镜像拉取失败、readiness 一直失败。确认流水线停在正确阶段，日志没有泄露 Secret，并且操作者能从构建记录定位到实际部署镜像。

自测：为什么重新构建同一个 `latest` 不能算可靠回滚？因为标签指向的内容可能已经改变，无法证明回滚镜像对应哪次源代码和构建过程。
