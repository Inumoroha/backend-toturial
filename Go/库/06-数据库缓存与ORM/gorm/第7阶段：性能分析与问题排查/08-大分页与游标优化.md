...
本节目标：理解大 offset 分页为什么慢，并掌握游标分页的基本实现。
...
```go
func ListByCursor(db *gorm.DB, lastID uint, limit int) ([]Article, error) {
...
这一节不要只停留在“看懂概念”。建议你按下面的方式把「大分页与游标优化」练成可以在项目中使用的能力。
