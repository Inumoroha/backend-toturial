...
本节目标：根据博客系统和订单系统的真实查询设计索引。索引不是背概念，而是为具体 SQL 服务。
...
```go
UserID    uint      `gorm:"index:idx_article_user_created"`
CreatedAt time.Time `gorm:"index:idx_article_user_created"`
```
...
```go
ArticleID uint      `gorm:"index:idx_comment_article_created"`
CreatedAt time.Time `gorm:"index:idx_comment_article_created"`
```
...
```go
UserID    uint      `gorm:"index:idx_order_user_created"`
CreatedAt time.Time `gorm:"index:idx_order_user_created"`
```
...
这一节不要只停留在“看懂概念”。建议你按下面的方式把「索引设计实战」练成可以在项目中使用的能力。
