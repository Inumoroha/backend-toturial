...
本节目标：理解哪些查询不应该每次都实时查数据库。GORM 优化不是只有索引，热点数据和统计数据常常需要缓存或预聚合。
...
这一节不要只停留在“看懂概念”。建议你按下面的方式把「缓存与预聚合思路」练成可以在项目中使用的能力。
