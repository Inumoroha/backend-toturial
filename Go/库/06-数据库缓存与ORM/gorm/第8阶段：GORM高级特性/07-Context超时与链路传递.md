...
本节目标：掌握 `WithContext` 的使用，把 HTTP 请求的 context 传递到 GORM 查询中。
...
不要在 Repository 里自己创建 `context.Background()`。
...
- 在 Repository 使用 `WithContext`。
- 从 Handler 传递请求 Context。
- 给查询设置超时。
...
这一节不要只停留在“看懂概念”。建议你按下面的方式把「Context超时与链路传递」练成可以在项目中使用的能力。
