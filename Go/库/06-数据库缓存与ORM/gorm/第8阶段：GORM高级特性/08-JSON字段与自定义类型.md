...
本节目标：掌握 GORM 保存 JSON 扩展字段的方式，并知道什么时候应该用 JSON，什么时候应该拆表。
...
var article Article
db.First(&article, id)
fmt.Println(article.Meta.CoverURL)
```
...
标签、评论、订单明细都不应该塞 JSON。

### 2. JSON 字段频繁筛选

如果经常按 JSON 内部字段查询，要重新考虑表设计。
...
这一节不要只停留在“看懂概念”。建议你按下面的方式把「JSON字段与自定义类型」练成可以在项目中使用的能力。
