...
本节目标：设计清晰的 Repository。Repository 不应该只是“把 GORM 换个地方写”，而应该表达数据访问能力。
...
`QueryArticles` 不如 `ListPublished` 或 `List` 加查询参数清楚。
...
- 设计清晰方法名。
- 为方法传入 Context。
- 让 Repository 支持事务中的 tx。

---

## 颗粒度补强：本节应该如何真正练会

这一节不要只停留在“看懂概念”。建议你按下面的方式把「Repository接口设计」练成可以在项目中使用的能力。
