...
本节目标：把 GORM 初始化、日志、连接池和迁移拆成独立数据库模块，避免 main 函数越来越臃肿。
...
### 1. 每个 Repository 都 gorm.Open

数据库连接应该启动时创建一次，然后注入。

### 2. 忽略连接池

真实服务必须配置连接池，否则高并发下容易出问题。

### 3. 迁移和初始化混在一起

拆成 `New` 和 `Migrate`，职责更清晰。
...
这一节不要只停留在“看懂概念”。建议你按下面的方式把「数据库模块设计」练成可以在项目中使用的能力。
