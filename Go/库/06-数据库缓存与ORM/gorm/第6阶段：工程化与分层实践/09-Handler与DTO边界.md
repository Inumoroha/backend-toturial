...
本节目标：掌握 Handler 的边界。Handler 只负责 HTTP 输入输出，不应该直接写 GORM，也不应该承载复杂业务。
...
func (h *ArticleHandler) Create(c *gin.Context) {
    ...
    article, err := h.service.CreateArticle(c.Request.Context(), service.CreateArticleInput{
...
### 1. Handler 直接 db.Create

这会让业务分散，测试困难。

### 2. Model 直接返回

可能暴露 `PasswordHash`、`DeletedAt` 等内部字段。

### 3. 前端传 user_id

当前用户应该来自 token，不来自请求体。
...
- 区分请求 DTO 和响应 DTO。
- 在 Handler 调用 Service。
- 避免 Handler 直接操作 GORM。
- 避免 Model 直接暴露给前端。
...
这一节不要只停留在“看懂概念”。建议你按下面的方式把「Handler与DTO边界」练成可以在项目中使用的能力。
