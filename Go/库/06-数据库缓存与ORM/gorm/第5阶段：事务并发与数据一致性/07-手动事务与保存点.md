...
本节目标：了解手动事务、嵌套事务和保存点。日常业务优先用闭包事务，但你需要能看懂手动事务代码。
...
GORM 支持嵌套事务。内部事务失败时，可以回滚内部部分。

但嵌套事务会增加理解成本。初中级项目里不要滥用。
...
这一节不要只停留在“看懂概念”。建议你按下面的方式把「手动事务与保存点」练成可以在项目中使用的能力。
